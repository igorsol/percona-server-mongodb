...
function doQueriesTest() {
    ...
    explain = t.find().sort({ _id : -1 }).explain("executionStats");
    assert.eq(1000, explain.executionStats.nReturned);
    ...
    // test range scan
    explain = t.find({ _id: { $gte : 500, $lte : 750 }  }).hint({_id : 1}).explain("executionStats");
    assert.eq(251, explain.executionStats.nReturned)
    ...
    // test reverse table scan
    explain = t.find({ _id: { $gte : 500, $lte : 750 }  }).sort({ _id : -1 }).hint({_id : 1}).explain("executionStats");
    assert.eq(251, explain.executionStats.nReturned)
    ...
    // test with a matcher
    explain = t.find({ _id: { $gte : 500, $lte : 750 }, a : {$gte : 1200}  }).hint({_id : 1}).explain("executionStats");
    assert.eq(251, explain.executionStats.totalKeysExamined)
    ...
    explain = t.find({ _id: { $gte : 500, $lte : 750 }, a : {$gte : 1200}  }).sort({ _id : -1 }).hint({_id : 1}).explain("executionStats");
    assert.eq(251, explain.executionStats.totalKeysExamined)
