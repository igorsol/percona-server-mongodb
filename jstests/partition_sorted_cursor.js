...
runQuery = function(query, index) {
    ...
    assert.eq("IXSCAN", explainOutput.queryPlanner.winningPlan.inputStage.stage);
...
runQueryWithProjection = function(query, projection, index) {
    ...
    assert.eq("FETCH", explainOutput.queryPlanner.winningPlan.inputStage.stage);
    // now let's compare the values
...
runQueryForFilter = function(query, index) {
    //printjson(index);
    explainOutput = t.find(query).hint(index).explain("executionStats");
    assert.eq("IXSCAN", explainOutput.queryPlanner.winningPlan.inputStage.stage);
    ...
    assert.eq(explainOutput.executionStats.nReturned, t.count(query));
    //print(t.count(query));
}

runSortedQueryForFilter = function(query, index, expectedNScanned) {
    //printjson(index);
    explainOutput = t.find(query).sort({a:1}).hint(index).explain("executionStats");
    assert.eq("IXSCAN", explainOutput.queryPlanner.winningPlan.inputStage.stage);
    // for each of these cursors, filtering should make nscanned the same as nscannedobjects
    // this means we need to be careful with queries we pass in
    //printjson(explainOutput);
    assert.eq(explainOutput.executionStats.nReturned, expectedNScanned);
...
*/
    ...
    runSortedQueryForFilter( {$or : [{ts : {$gte: 81}}, {ts : {$lte : 20}}]}, index, 40 ); // non-overlapping or
